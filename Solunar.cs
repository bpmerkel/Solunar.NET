...
public class SolunarClass
{
    ...
    public static SolunarInfo Solunar(DateTime date, double latitude, double longitude)
    {
    ...
    private static List<SolunarRange> CalculateMajorMinorPeriods(List<TransitInfo> transitTimes, DateTime? moonrise, DateTime? moonset, DateTime sunrise, DateTime sunset)
    {
    ...
    private static MoonInfo MoonData(DateTime start, DateTime end, double latitude, double longitude)
    {
    ...
    private static List<TransitInfo> MoonTransitTimes(DateTime start, double latitude, double longitude)
    {
    ...
    private static int CalculateDayScore(List<SolunarRange> solunarPeriods, string phaseName, double phase)
    {
        var dayScore = solunarPeriods.Sum(r => r.Weight);
        if (phaseName is NewMoon or FullMoon)
        {
            dayScore += 3;
        }
        else
    ...
    private static string MoonPhaseName(double phase) => new[]
    {
